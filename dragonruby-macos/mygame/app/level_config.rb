...
module LevelConfig
  ...
  # Per-level rows: [ghost_speed, ghost_tunnel, enrage1_dots, enrage1_speed,
  #                  enrage2_dots, enrage2_speed]. Index 0 is unused (level 1 = [1]).
  # The enrage* dot thresholds are per-Territory dots-remaining (ADR-0010).
  # Each Territory holds ~60 dots, so enrage1 ≈ 33% cleared, enrage2 ≈ 67%
  # cleared at L1; both ramp earlier as the level rises. Curve still rough —
  # G6 per-level tuning pass deferred.
  ROWS = [
    nil,
    [0.75, 0.40, 40, 0.85, 20, 0.95], # 1
    [0.85, 0.45, 45, 0.90, 22, 0.95], # 2
    [0.85, 0.45, 45, 0.90, 25, 0.95], # 3
    [0.85, 0.45, 45, 0.90, 25, 0.95], # 4
    [0.95, 0.50, 48, 1.00, 27, 1.05], # 5
    [0.95, 0.50, 48, 1.00, 27, 1.05], # 6
    [0.95, 0.50, 50, 1.00, 28, 1.05], # 7
    [0.95, 0.50, 50, 1.00, 28, 1.05], # 8
    [0.95, 0.50, 50, 1.00, 30, 1.05], # 9
    [0.95, 0.50, 50, 1.00, 30, 1.05], # 10
    [0.95, 0.50, 52, 1.00, 32, 1.05], # 11
    [0.95, 0.50, 52, 1.00, 32, 1.05], # 12
    [0.95, 0.50, 52, 1.00, 33, 1.05], # 13
    [0.95, 0.50, 52, 1.00, 33, 1.05], # 14
    [0.95, 0.50, 55, 1.00, 35, 1.05], # 15
    [0.95, 0.50, 55, 1.00, 35, 1.05], # 16
    [0.95, 0.50, 55, 1.00, 36, 1.05], # 17
    [0.95, 0.50, 55, 1.00, 36, 1.05], # 18
    [0.95, 0.50, 56, 1.00, 38, 1.05], # 19
    [0.95, 0.50, 56, 1.00, 38, 1.05], # 20
    [0.95, 0.50, 56, 1.00, 40, 1.05]  # 21+
  ].freeze
